...
class PoemInput extends Component {
  ...
  handleChange(event) {
    ...
    const lastWord = lastElem[this.state.text.split(punctuationRegEx).filter(v => v)]
      .replace("'", "’")
    ...
    return (
      <div id='entry-container'>
        ...
        <SuggestionBox filters={this.props.filters} 
          add={this.add} 
          style={{position: 'absolute', top: `${this.state.coords.top + 190}px`, left: `${this.state.coords.left + 20}px`}} />
       </div>
    )
  }
}
