...
const getCaretCoordinates = (element, position, extra) => {
  // mirrored div
  ...
  if (!extra) extra = ''
  mirrorDiv.textContent = element.value.substring(0, position) + extra;
  ...
  span.textContent = element.value.substring(position) + extra || '.';  // || because a completely empty faux span doesn't render at all
